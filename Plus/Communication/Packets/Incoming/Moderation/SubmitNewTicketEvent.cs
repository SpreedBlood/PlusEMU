...
namespace Plus.Communication.Packets.Incoming.Moderation
{
    ...
    {
        public int Header => ClientPacketHeader.SubmitNewTicketMessageEvent;
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            ModerationTicket ticket = new ModerationTicket(1, type, category, UnixTimestamp.GetNow(), 1, session.Habbo, reportedUser, message, session.Habbo.CurrentRoom, chats);
            ...
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                /*dbClient.SetQuery("INSERT INTO `moderation_tickets` (`score`,`type`,`status`,`sender_id`,`reported_id`,`moderator_id`,`message`,`room_id`,`room_name`,`timestamp`) VALUES (1, '" + Category + "', 'open', '" + Session.Habbo.Id + "', '" + ReportedUserId + "', '0', @message, '0', '', '" + PlusEnvironment.GetUnixTimestamp() + "')");
                dbClient.AddParameter("message", Message);
                dbClient.RunQuery();*/

                dbClient.RunQuery("UPDATE `user_info` SET `cfhs` = `cfhs` + '1' WHERE `user_id` = '" + session.Habbo.Id + "' LIMIT 1");
            }
